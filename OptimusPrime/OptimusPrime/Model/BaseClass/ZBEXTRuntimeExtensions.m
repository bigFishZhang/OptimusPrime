...
zb_propertyAttributes *zb_copyPropertyAttributes (objc_property_t property) {
    ...
    zb_propertyAttributes *attributes = calloc(1, sizeof(zb_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate zb_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = zb_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = zb_propertyMemoryManagementPolicyRetain;
                break;
