...
- (void)_setupNavigationBarBottomLine{
    ...
    UIImageView *navSystemLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.navigationBar.zb_height - navSystemLineH, ZB_SCREEN_WIDTH, navSystemLineH)];
    navSystemLine.backgroundColor = ZBColor(223.0f, 223.0f, 221.0f);
...
+ (void) _setupNavigationBarTheme{
    ...
    textAttrs[NSFontAttributeName] = ZBMediumFont(18.0f);
...
+ (void)_setupBarButtonItemTheme{
    ...
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = [UIColor whiteColor];
    textAttrs[NSFontAttributeName] = ZBRegularFont(fontSize);
    NSShadow *shadow = [[NSShadow alloc] init];
    shadow.shadowOffset =  CGSizeZero;
    textAttrs[NSShadowAttributeName] = shadow;
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if (self.viewControllers.count > 0){
        ...
        if ([viewController isKindOfClass:[ZBViewController class]]) {
            
            ZBViewModel *viewModel = [(ZBViewController *)viewController viewModel];
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem zb_backItemWithTitle:title imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_back)];
    }
    // push
