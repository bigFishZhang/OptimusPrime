...
/// tableView
@property (nonatomic, readwrite, weak)   ZBTableView *tableView;
...
/// init
- (instancetype)initWithViewModel:(ZBTableViewModel *)viewModel {
...
- (void)_su_setupSubViews{
    ...
    ZBTableView *tableView = [[ZBTableView alloc] initWithFrame:[UIScreen mainScreen].bounds style:self.viewModel.style];
    tableView.backgroundColor = ZB_MAIN_BACKGROUNDCOLOR;
    ...
    if (self.viewModel.shouldPullDownToRefresh) {
        /// 下拉刷新
        @weakify(self);
        [self.tableView zb_addHeaderRefresh:^(MJRefreshNormalHeader *header) {
            /// 加载下拉刷新的数据
            @strongify(self);
    ...
    if (self.viewModel.shouldPullUpToLoadMore) {
        /// 上拉加载
        @weakify(self);
        [self.tableView zb_addFooterRefresh:^(MJRefreshAutoNormalFooter *footer) {
            /// 加载上拉刷新的数据
            @strongify(self);
    ...
    if (@available(iOS 11.0, *)) {
        ...
        ZBAdjustsScrollViewInsets_Never(tableView);
