...
/// KVO 监听的属性
/// 加载情况
static NSString * const ZBWebViewKVOLoading = @"loading";
/// 文章标题
static NSString * const ZBWebViewKVOTitle = @"title";
/// 进度
static NSString * const ZBWebViewKVOEstimatedProgress = @"estimatedProgress";
...
/// viewModel
@property (nonatomic, strong, readonly) ZBWebViewModel *viewModel;
...
- (void)viewDidLoad {
    ...
    if (!(ZBIOSVersion>=9.0)) [[NSUserDefaults standardUserDefaults] registerDefaults:@{@"userAgent":userAgent}];
    ...
    WKWebView *webView = [[WKWebView alloc] initWithFrame:ZB_SCREEN_BOUNDS configuration:configuration];
    webView.navigationDelegate = self;
    webView.UIDelegate = self;
    
    if ((ZBIOSVersion >= 9.0)) webView.customUserAgent = userAgent;
    ...
    [_KVOController zb_observe:self.webView keyPath:ZBWebViewKVOTitle block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        @strongify(self);
    ...
    [_KVOController zb_observe:self.webView keyPath:ZBWebViewKVOLoading block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        NSLog(@"--- webView is loading ---");
    }];
    
    [_KVOController zb_observe:self.webView keyPath:ZBWebViewKVOEstimatedProgress block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        @strongify(self);
    ...
    if(self.viewModel.shouldPullDownToRefresh){
        [self.webView.scrollView zb_addHeaderRefresh:^(MJRefreshNormalHeader *header) {
            @strongify(self);
            [self.webView reload];
        }];
    ...
    if (@available(iOS 11.0, *)) {
        ZBAdjustsScrollViewInsets_Never(webView.scrollView);
    }
}
...
- (UIEdgeInsets)contentInset{
    return UIEdgeInsetsMake(ZB_APPLICATION_TOP_BAR_HEIGHT, 0, 0, 0);
}
...
//    [NSObject zb_showAlertViewWithTitle:nil message:message confirmTitle:@"我知道了"];
    completionHandler();
}
...
- (UIProgressView *)progressView {
    if (!_progressView) {
        CGFloat progressViewW = ZB_SCREEN_WIDTH;
        ...
        progressView.progressTintColor = ZB_MAIN_TINTCOLOR;
...
{
    if (_backItem == nil) {
        _backItem = [UIBarButtonItem zb_backItemWithTitle:@"返回" imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_backItemDidClicked)];
    }
    return _backItem;
}
...
- (UIBarButtonItem *)closeItem {
    if (!_closeItem) {
        _closeItem = [UIBarButtonItem zb_systemItemWithTitle:@"关闭" titleColor:nil imageName:nil target:self selector:@selector(_closeItemDidClicked) textType:YES];
    }
    return _closeItem;
}
